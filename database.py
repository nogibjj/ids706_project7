...
def insert_data(cursor,connection,sql_insertion,i1,i2,i3):
    cursor.execute(sql_insertion,(i1,i2,i3,))
    connection.commit()
...
def update_data(cursor,connection,query_update,stock,name):
    cursor.execute(query_update,(stock,name,))
    connection.commit()
...
def main():
    ...
    insert_data(cursor,connection,query_insertion,"Red Chamber",30,"Uncopiable Love Story")
    insert_data(cursor,connection,query_insertion,"Educated",35,"Meaning of Education")
    # R: read from table
